...
def inference_set(inf_loader, model, device, best_, save_model=False, suffix='s', set_name="na"):
    ...
    for i, datas in enumerate(tqdm(inf_loader, desc="Validating")):
        ...
        #video["extracted_frames"] = data["extracted_frames"].to(device)
        #video["blur_feature"] = data["blur_feature"].to(device)
        with torch.no_grad():
...
def main():
    ...
    parser.add_argument(
        "-o", "--opt", type=str, default="./options/fast/old_vs_resize.yml", help="the option file"
    )
